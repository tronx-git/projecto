...
$sql = "SELECT * FROM inventory WHERE product_type LIKE '%reinforced%'";
$result = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>Steel reinforced Pipes</title>
    <meta charset="utf-8">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while($row = $result->fetch_assoc()): ?>
                        <div class="col-md-6 mb-4">
                            <img src="<?php echo !empty($row['image']) ? htmlspecialchars($row['image']) : 'images/non-arm-pipes.png'; ?>" alt="<?php echo !empty($row['name']) ? htmlspecialchars($row['name']) : 'Standard non armored pipes'; ?>" class="img-fluid">
                        </div>
                        <div class="col-md-6 mb-4">
                            <!-- Remove the h1 title -->
                            <!-- <h1><?php echo !empty($row['name']) ? htmlspecialchars($row['name']) : 'Standard Non Armored Pipes'; ?></h1> -->
