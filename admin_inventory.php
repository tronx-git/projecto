...
if (isset($_POST['login'])) {
    $user = $_POST['username'] ?? '';
    $pass = $_POST['password'] ?? '';
    if ($user === '---redacted--' && $pass === '---redacted--') {
...
if (isset($_POST['add_new'])) {
    $stmt = $conn->prepare("INSERT INTO inventory (product, product_type, items_in_stock, items_requested, picture, `3d_image`, `tech_drawing`, price, diameter, length, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['delete_id'])) {
    ...
    $conn->query("DELETE FROM inventory WHERE id = $del_id");
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}
...
if (isset($_POST['edit_id'])) {
    ...
    $edit_row = $conn->query("SELECT * FROM inventory WHERE id = $edit_id")->fetch_assoc();
}

// Handle Save Edit
if (isset($_POST['save_edit'])) {
    $stmt = $conn->prepare("UPDATE inventory SET product=?, product_type=?, items_in_stock=?, items_requested=?, picture=?, `3d_image`=?, `tech_drawing`=?, price=?, diameter=?, length=?, description=? WHERE id=?");
    // "sssisssdsssi"
    $stmt->bind_param(
        "sssisssdsssi",
        $_POST['product'],
